...
\lstset{
  ...
  keywordstyle=\color{kw}\bfseries,   % 关键词变色加粗
  stringstyle=\color{st},
  commentstyle=\color{cm}\itshape,
  identifierstyle=\color{black},
  numberstyle=\color{nm}\small,      % 行号加大
  numbers=left, numbersep=8pt,
  showstringspaces=false, tabsize=2, frame=none,
}
...
\lstalias{tex}{latex}
% 注意：不要写 \lstalias{sh}{bash}，它会触发 listings 的 \lstKV@OptArg 无限递归（栈溢出）。
% 想用 shell 就写 language=bash 即可。
